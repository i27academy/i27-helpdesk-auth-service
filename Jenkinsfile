def deployToGKE(String namespace, String envLabel) {
    ...
    // where for example,auth-db-jdbc-url is your id/description and place the actual value
    // where DB_JDBC_URL is the variable calling, and the same name should be available in secret.yaml file 
    withCredentials([
...
pipeline {
    ...
    environment {
        ...
        IMAGE_REPOSITORY = "devopswithcloudhub/auth-service"
